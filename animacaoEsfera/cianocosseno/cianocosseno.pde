...
void projecao(double a, double b,double c,double r,double xc,double yc,double zc,double p,double q,double w,int cont){
  int i, j, m; //ponto no plano de projeção(i,j,0) 
  double A, B, C, Delta;
  ...
  for (i=-300; i<300; i++){
    for (j=-200; j<200; j++){
      ...
      if (Delta<0.0) { img.set(i+300,j+200,color(255,255,255,0)); }
        ...
        img.set(i+300,j+200,color(0,m,m,255));
      }
   }
  } 
 
  img.save("c"+str(cont)+".png");
  exit();
}
...
void setup() {
  ...
  double p  = -300.0, q = 300.0, w  = -120.0;            //luz(p,q,w)
