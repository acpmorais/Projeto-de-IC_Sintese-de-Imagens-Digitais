...
void setup() {
  ...
  header += "end_header";
  output = createWriter("MickeyZX.ply"); 
  ...
  projecaoParalelaElipsoideZX(78,79,23,18,25,16,p,q,w,"yellow");
  projecaoParalelaElipsoideZX(78,108,26,10,12,9,p,q,w,"cyan");
...
void projecaoParalela(double a, double b, double c, double r, double p, double q, double w, String cor) {
  ...
  for (i = 0; i < 600; i++) {
    for (k = 0; k < 400; k++) {
      ...
      //y = t
      //z = k
      //(i-a)^2 + (t-b)^2 + (k-c)^2 = r^2
      //(t-b)^2  = r^2 - ( (i-a)^2 + (k-c)^2 )
      //(t-b)    = raiz( r^2 - ( (i-a)^2 + (k-c)^2 ) )
      // t       = b + raiz( r^2 - ( (i-a)^2 + (k-c)^2 ) )
      delta = (r*r) - (((i-a)*(i-a)) + ((k-c)*(k-c)));
      if (delta >= 0) {
        t = raiz(delta) + b;
        ...
        if      (cor == "black" ) { img.set(i, k, color(0, 0, 0));  output.println(i+"\t"+k+"\t0\t0\t0\t0"); }
        else if (cor == "blue"  ) { img.set(i, k, color(0, 0, m));  output.println(i+"\t"+k+"\t0\t0\t0\t255"); }
        else if (cor == "green" ) { img.set(i, k, color(0, m, 0));  output.println(i+"\t"+k+"\t0\t0\t255\t0"); }
        else if (cor == "cyan"  ) { img.set(i, k, color(0, m, m));  output.println(i+"\t"+k+"\t0\t0\t255\t255"); }
        else if (cor == "red"   ) { img.set(i, k, color(m, 0, 0));  output.println(i+"\t"+k+"\t0\t255\t0\t0"); }
        else if (cor == "mag"   ) { img.set(i, k, color(m, 0, m));  output.println(i+"\t"+k+"\t0\t255\t0\t255"); }
        else if (cor == "yellow") { img.set(i, k, color(m, m, 0));  output.println(i+"\t"+k+"\t0\t255\t255\t0");}
        else if (cor == "white" ) { img.set(i, k, color(m, m, m));  output.println(i+"\t"+k+"\t0\t255\t255\t255");}
...
void projecaoParalelaElipsoideZX(double a, double b, double c, double f, double g, double h, double p, double q, double w, String cor) {
  int i, j, k, m;
  double delta, t;
  ...
  for (i = 0; i < 600; i++) {
    for (k = 0; k < 400; k++) {
      ...
      //y = t
      //z = k
      //((i-a)^2)/f^2 + ((t-b)^2)/g^2 + ((k-c)^2)/h^2 = 1
      // ((t-b)^2)/g^2 +  = 1 - ((i-a)^2)/f^2 + ((k-c)^2)/h^2 )
      // ((t-b)^2)/g^2 +  = (1 - ((i-a)^2)/f^2 + ((k-c)^2)/h^2 ) )
      // ((t-b)^2) +  = g^2*(1 - ((i-a)^2)/f^2 + ((k-c)^2)/h^2 ) )
      // t-b         = g*raiz(1 - ((i-a)^2)/f^2 + ((k-c)^2)/h^2 ) )
      // t = b + g*raiz(1 - ((i-a)^2)/f^2 + ((k-c)^2)/h^2 ) )
      
      delta = (1 - (((i-a)*(i-a))/(f*f)) - (((k-c)*(k-c))/(h*h)));
      if (delta >= 0) {
        t = (g*raiz(delta)) + b;
        ...
        if      (cor == "black")  { img.set(i, k, color(0, 0, 0));  } // BLACK: 000 : 0
        else if (cor == "blue" )  { img.set(i, k, color(0, 0, m));  } // BLUE:: 001 : 1
        else if (cor == "green")  { img.set(i, k, color(0, m, 0));  } // GREEN: 010 : 2
        else if (cor == "cyan" )  { img.set(i, k, color(0, m, m));  } // CYAN:: 011 : 3
        else if (cor == "red"  )  { img.set(i, k, color(m, 0, 0));  } // RED::: 100 : 4
        else if (cor == "mag"  )  { img.set(i, k, color(m, 0, 1));  } // MAGEN: 101 : 5
        else if (cor == "yellow") { img.set(i, k, color(m, m, 0));  } // YELLO: 110 : 6
        else if (cor == "white")  { img.set(i, k, color(m, m, m));  } // WHITE: 111 : 7
